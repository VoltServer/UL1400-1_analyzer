...
Module Attributes:
  FORMAT_TYPE_NAMES: All possible names for all supported formats.
  SOURCE_TYPE_NAMES: The possible names that can be used as the source type to
    invoke this data importer.
  _FORMAT_TYPE_GROUPS: All the groups of format type names for the various
    formats supported.
  _FORMAT_TYPE_NAMES_CSV: The possible names that can be used as the format type
...
FORMAT_TYPE_NAMES = [f for g in _FORMAT_TYPE_GROUPS for f in g]
...
def import_data(format_type:str, *args:Any, **kwargs:Any) -> Any:
    ...
    """
    if format_type.lower() in _FORMAT_TYPE_NAMES_CSV:
